...
/**
 * TASK-421 live-wire proof for inventory hedging through ParallelVenueExecution.
 *
 * <p>The fixture runs the production {@link InventoryHedgeStrategy} and
 * {@link ParallelVenueExecution} against local Coinbase and Binance simulators.
 * A threshold breach emits a hedge parent with {@code venueSetId=7}; the
 * execution plugin creates one IOC child per venue, the gateway command handler
 * routes decoded SBE commands to each simulator socket, simulator execution
 * reports are normalized back into SBE, and parent-terminal callbacks clear the
 * strategy active state and apply cooldown. The test is local-only evidence:
 * no live exchange endpoint, REST recovery, external credentials, or QA/UAT
 * network access is used.</p>
 */
@Tag("E2E")
final class InventoryHedgeParallelVenueLiveWireE2ETest {
    private static final long BTC = Ids.SCALE;
    private static final long QTY = BTC;
    private static final long PRICE = 65_000L * Ids.SCALE;

    @Test
    void thresholdTriggerParallelChildrenPartialFillsAndParentCallbackComplete() throws Exception {
        final KeyPair keyPair = KeyPairGenerator.getInstance("Ed25519").generateKeyPair();
        try (LiveWire live = liveWire(CoinbaseExchangeSimulator.FillMode.PARTIAL_THEN_FULL,
            BinanceExchangeSimulator.FillMode.PARTIAL_THEN_FULL, false, keyPair)) {
            live.logon();
            ...
            waitUntil(() -> live.coinbase.executionReports().stream()
                .anyMatch(report -> "2".equals(report.ordStatus())));
            waitUntil(() -> live.binance.executionReports().stream()
                .anyMatch(report -> "2".equals(report.ordStatus())));
            assertThat(live.coinbase.executionReports()).extracting(CoinbaseExchangeSimulator.ExecutionReport::ordStatus)
                .contains("1", "2");
            assertThat(live.binance.executionReports()).extracting(BinanceExchangeSimulator.ExecutionReport::ordStatus)
                .contains("1", "2");

            live.driveCoinbase(parentOrderId + 1L, Side.SELL);
            ...
            assertThat(live.parallel.childSubmissions()).isEqualTo(2L);
            assertThat(live.strategy.activeParentOrderId()).isZero();
            assertThat(live.strategy.cooldownUntilMicros()).isGreaterThan(live.cluster.time);
            assertThat(live.terminalReasons).containsExactly(ParentTerminalReason.COMPLETED);
            assertThat(live.coinbase.receivedOrders()).hasSize(1);
            assertThat(live.binance.receivedOrders()).hasSize(1);
        }
    }

    @Test
    void timerDrivenResidualCancelExpiresParentAndAppliesCooldown() throws Exception {
        final KeyPair keyPair = KeyPairGenerator.getInstance("Ed25519").generateKeyPair();
        try (LiveWire live = liveWire(CoinbaseExchangeSimulator.FillMode.IMMEDIATE,
            ...
            live.coinbaseClient.readMessageContaining("150=F");
            live.driveCoinbase(parentOrderId + 1L, Side.SELL);
            live.engine.onTimer(live.parallelTimerCorrelationId(parentOrderId));

            assertThat(live.parents.lookup(parentOrderId).status()).isEqualTo(ParentOrderState.EXPIRED);
            ...
            assertThat(live.parallel.timerFirings()).isEqualTo(1L);
            assertThat(live.parallel.residualCancels()).isEqualTo(1L);
            assertThat(live.strategy.activeParentOrderId()).isZero();
            assertThat(live.terminalReasons).containsExactly(ParentTerminalReason.EXECUTION_ABORTED);
        }
    }

    @Test
    void allChildrenRejectedBySimulatorsFailsParentAndFailureCooldown() throws Exception {
        final KeyPair keyPair = KeyPairGenerator.getInstance("Ed25519").generateKeyPair();
        try (LiveWire live = liveWire(CoinbaseExchangeSimulator.FillMode.REJECT_ALL,
            ...
            live.route(parentOrderId + 1L);
            live.route(parentOrderId + 2L);
            live.coinbaseClient.readMessageContaining("150=8");
            ...
            assertThat(live.parents.lookup(parentOrderId).terminalReasonCode())
                .isEqualTo(ParentOrderState.REASON_ALL_CHILDREN_REJECTED);
            assertThat(live.parallel.allChildrenRejected()).isEqualTo(1L);
    ...
    @Test
    void parentCancelMidFlightCancelsChildrenAndClearsActiveState() throws Exception {
        final KeyPair keyPair = KeyPairGenerator.getInstance("Ed25519").generateKeyPair();
        try (LiveWire live = liveWire(CoinbaseExchangeSimulator.FillMode.NO_FILL,
            BinanceExchangeSimulator.FillMode.NO_FILL, false, keyPair)) {
            live.logon();
            live.triggerLongExposure();
            final long parentOrderId = live.strategy.activeParentOrderId();
            ...
            live.route(parentOrderId + 1L);
            live.route(parentOrderId + 2L);
            live.engine.cancelParent(parentOrderId, ParentOrderState.REASON_CANCELED_BY_PARENT);

            assertThat(live.parents.lookup(parentOrderId).status()).isEqualTo(ParentOrderState.CANCELED);
            assertThat(live.parents.activeChildCount(parentOrderId)).isZero();
            assertThat(live.parallel.parentCancels()).isEqualTo(1L);
            assertThat(live.strategy.activeParentOrderId()).isZero();
            assertThat(live.terminalReasons).containsExactly(ParentTerminalReason.CANCELED_BY_PARENT);
        }
    }
    ...
    private static LiveWire liveWire(
        ...
        return new LiveWire(coinbase(coinbaseMode), binance(binanceMode, keyPair), fixture, strategy, keyPair);
    }
    ...
    private static Fixture fixture(final boolean rejectRisk) {
        ...
        final ParallelVenueExecution parallel = new ParallelVenueExecution(1L, 16);
        ...
        return new Fixture(marketView, parents, orders, risk, portfolio, cluster, commandBuffer, parallel, engine,
    ...
    private static ExecutionEventDecoder execution(
        ...
        final byte[] execId = ("parallel-" + childClOrdId + '-' + venueId + '-' + report.execType())
    ...
    private static void waitUntil(final java.util.function.BooleanSupplier condition) throws InterruptedException {
    ...
    private record Fixture(
        ...
        ParallelVenueExecution parallel,
    ...
    private static final class LiveWire implements AutoCloseable {
        ...
        final ParallelVenueExecution parallel;
        ...
        void triggerLongExposure() {
            fixture.marketView.apply(marketData(Ids.VENUE_COINBASE, EntryType.BID, PRICE), cluster.time);
            fixture.marketView.apply(marketData(Ids.VENUE_COINBASE, EntryType.ASK, PRICE), cluster.time);
            fixture.marketView.apply(marketData(Ids.VENUE_BINANCE, EntryType.BID, PRICE), cluster.time);
            fixture.marketView.apply(marketData(Ids.VENUE_BINANCE, EntryType.ASK, PRICE), cluster.time);
        ...
        long parallelTimerCorrelationId(final long parentOrderId) {
            return parentOrderId + 10_000L;
        }
    ...
    private static final class RecordingTimerScheduler implements ExecutionStrategyContext.TimerScheduler {
        ...
        public boolean scheduleTimer(final long correlationId, final long deadlineClusterMicros) {
            return scheduleTimer(correlationId, deadlineClusterMicros, ExecutionStrategyIds.PARALLEL_VENUE);
        }
        ...
        RecordingCluster() {
            proxy = (Cluster)Proxy.newProxyInstance(Cluster.class.getClassLoader(), new Class<?>[] {Cluster.class},
                (p, m, a) -> switch (m.getName()) {
                    ...
                    case "toString" -> "InventoryHedgeParallelVenueLiveWireCluster";
                    default -> null;
                });
        }
    }
    ...
    private record CoinbaseReportView(CoinbaseExchangeSimulator.ExecutionReport report) implements ReportView {
        ...
        @Override public String ordStatus() { return report.ordStatus(); }
